...
- Status: resolved. `.prose` is now the scoped rich-text implementation.
- `.prose-sm`, `.prose-md`, `.prose-lg`, and `.prose-xl` control width only.
- `.prose-emily` is not generated by default; enable `prose.legacyAlias` only for temporary backwards compatibility.
...
v1.4.0 housekeeping:
- Keep `.prose` as the public rich-text class and avoid default `.prose-emily` output
