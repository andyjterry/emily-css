...
```json
{
  "prose": {
    ...
    "legacyAlias": false,
    "elements": {
      "p": { "color": "neutral-80", "lineHeight": 1.75 },
      "h2": {
        "fontSize": "3xl",
        "lineHeight": "3xl",
        "color": "neutral-90",
...
Generates scoped rich-text styles for `.prose` only. EmilyCSS does not generate global `p`, `h2`, or list styles from this section.

Width modifiers are always Tailwind-compatible: `.prose-sm`, `.prose-md`, `.prose-lg`, and `.prose-xl`. These only control `max-width`; use typography utilities such as `text-lg` or `text-xl` for font size.

Set `"enabled": false` to disable the prose layer. Set `"legacyAlias": true` only if an existing project still needs `.prose-emily`; it is not generated by default.

Example:

```html
<article class="prose prose-lg text-xl p-4 mx-auto">
  ...
</article>
```
