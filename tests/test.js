...
// ─── 17. Softened Plain Colours, Prose, Focus Helpers ────────────────────────

section('17. Softened Plain Colours, Prose, Focus Helpers');
...
test('patternComponents generates scoped .prose rich-text styles by default', () => {
  const { patternComponents } = require('../src/generators/patterns.js');
  const css = patternComponents({});

  assert.ok(css.includes('.prose {'), 'Missing .prose wrapper');
  assert.ok(
    css.includes('max-width: var(--prose-width-md, 65ch);'),
    'Default .prose width should use md prose width'
  );
  assert.ok(css.includes('.prose p {'), 'Missing scoped prose paragraph styles');
  assert.ok(css.includes('.prose h2 {'), 'Missing scoped prose heading styles');
  assert.ok(!/^\s*p\s*\{/m.test(css), 'Prose should not generate global paragraph selectors');
});

test('patternComponents does not generate .prose-emily by default', () => {
  const { patternComponents } = require('../src/generators/patterns.js');
  const css = patternComponents({});

  assert.ok(!css.includes('.prose-emily'), 'prose-emily should not be generated by default');
});

test('patternComponents generates prose width modifiers for width only', () => {
  const { patternComponents } = require('../src/generators/patterns.js');
  const css = patternComponents({});

  ['sm', 'md', 'lg', 'xl'].forEach((size) => {
    assert.ok(css.includes(`.prose-${size} {`), `Missing .prose-${size}`);
  });
  assert.ok(css.includes('.prose-sm {\n    max-width: 55ch;'), '.prose-sm should set width');
  assert.ok(css.includes('.prose-xl {\n    max-width: 85ch;'), '.prose-xl should set width');
  assert.ok(!css.includes('.prose-xl {\n    font-size:'), '.prose-xl should not set font size');
});

test('patternComponents respects custom prose widths and defaultWidth', () => {
  const { patternComponents } = require('../src/generators/patterns.js');
  const css = patternComponents({
    prose: {
      defaultWidth: 'lg',
  ...
  assert.ok(
    css.includes('max-width: var(--prose-width-lg, 70ch);'),
    'Default prose width should follow prose.defaultWidth'
  );
  assert.ok(css.includes('.prose-lg {\n    max-width: 70ch;'), 'Custom prose-lg width missing');
});

test('patternComponents can generate optional .prose-emily legacy alias', () => {
  const { patternComponents } = require('../src/generators/patterns.js');
  const css = patternComponents({ prose: { legacyAlias: true } });

  assert.ok(css.includes('.prose-emily'), 'legacyAlias should generate .prose-emily');
  assert.ok(css.includes('.prose-emily p'), 'legacyAlias should scope child prose styles');
});

test('patternComponents disables prose when prose.enabled is false', () => {
  const { patternComponents } = require('../src/generators/patterns.js');
  const css = patternComponents({ prose: { enabled: false } });

  assert.ok(!css.includes('.prose {'), '.prose should not be generated when disabled');
  assert.ok(!css.includes('.prose-sm'), 'width modifiers should not be generated when prose is disabled');
});
